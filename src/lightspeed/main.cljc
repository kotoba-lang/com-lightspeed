(ns lightspeed.main
  "Kotodama WASM entrypoint for the Lightspeed clean-room actor (L4) — Clojure port.
  ...
  facts (`lightspeed.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lightspeed")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "lightspe_pro"
   ...
   {:entity "Customer"  :plural "customers" :id-prefix "lightspe_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {} :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "lightspe_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"lightspeed.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lightspeed-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lightspeed-compat/ready)
